"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.NoticesService = void 0;
const typeorm_1 = require("@nestjs/typeorm");
const common_1 = require("@nestjs/common");
const notices_repository_1 = require("./notices.repository");
const notice_entity_1 = require("./entities/notice.entity");
let NoticesService = class NoticesService {
    constructor(noticesRepository) {
        this.noticesRepository = noticesRepository;
    }
    async createNotice(createNoticeDto) {
        const result = await this.noticesRepository.save(Object.assign({}, createNoticeDto));
        return result;
    }
    async getNoticeList() {
        const result = await this.noticesRepository.find();
        return result;
    }
    async getNotice(id) {
        const result = await this.noticesRepository.findOne({ id });
        return result;
    }
    async updateNotice(updateNoticeDto, id) {
        const result = await this.noticesRepository
            .createQueryBuilder('notice')
            .update(notice_entity_1.Notice)
            .set(Object.assign({}, updateNoticeDto))
            .where('id = :id', { id })
            .execute();
        return result;
    }
    async deleteNotice(id) {
        return await this.noticesRepository.delete(id);
    }
};
NoticesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(notices_repository_1.NoticesRepository)),
    __metadata("design:paramtypes", [notices_repository_1.NoticesRepository])
], NoticesService);
exports.NoticesService = NoticesService;
//# sourceMappingURL=notices.service.js.map